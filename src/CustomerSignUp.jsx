...
function CustomerSignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Replace this with your actual signup request (e.g. fetch/axios call)
    ...
    <div className="h-screen w-full flex items-center justify-center p-6" style={{ backgroundImage: "url('src/assets/page_background.jpg')", backgroundSize: "cover", backgroundAttachment: "fixed", backgroundPosition: "center"}}>
        ...
        <div className="w-full max-w-5xl h-[600px] flex flex-col md:flex-row items-stretch  bg-white/95 rounded-2xl shadow-xl overflow-hidden border border-green-950">              
            ...
            <div style={{ flex: 0.8}} className="signup-left-card relative bg-white/90 rounded-2xl p-8 md:p-6 flex flex-col justify-center">
                ...
                <h2 className="text-2xl font-bold text-green-800 mb-1 mt-4">Create your account</h2>
                ...
                <form onSubmit={handleSubmit} className="signup-form flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1 mb-0.9"> 
                        <label htmlFor="password" className="text-xs font-semibold text-green-800" > Password </label> {/* Password input with eye button */}         
                            <div className="relative">
                                <Lock
                                    size={18}
                                    className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"
                            ...
                            <input type={showPassword ? 
                                   "text" : "password"} 
                                    ...
                                    onChange={handleChange} 
                                    required className={`${inputClass} pr-10 pl-10`} 
                            /> 
                            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-green-800" > {showPassword ? <Eye /> : <EyeOff />} 
                            </button> 
                        </div> 
                        
                            <h4 className="text-xs text-gray-600"> Use 8+ characters with a mix of letters, numbers, and symbols </h4> {/* Terms and Privacy checkbox */} 
                            <div className="flex items-start gap-2"> 
                                <input type="checkbox" 
                                        ...
                                        onChange={handleChange} 
                                        required className="w-4 h-4 accent-green-700 cursor-pointer" 
                                /> 
                    ...
                    {status.error && (
                    <p className="login-error text-s text-red-600">{status.error}</p>
                    )}
                    {status.success && (
                    <p className="login-success text-s text-green-700">{status.success}</p>
                    )}
                    ...
                    <button
                        type="button"
                        className="google-signup-button w-full flex items-center justify-center gap-2 border border-gray-300 rounded-lg py-3 text-xs font-medium hover:bg-gray-50 transition mb-3"
                        >
